...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _showBuyFlow(dynamic weapon) async {
    final result = await showDialog<Map<String, dynamic>?>(
      ...
      builder: (_) => _BuyDialog(weapon: weapon),
    );
    ...
    if (result['success'] == true) {
      ...
      if (remaining != null) {
        setState(() => _user = {..._user, 'coin': remaining});
      }
      _showResultDialog(
        success: true,
        title: 'Pembelian Berhasil!',
        subtitle: remaining != null
            ? 'Sisa koin: ${formatCoins(remaining)}'
            : null,
      );
    } else {
      _showResultDialog(
...
class _BuyDialog extends StatefulWidget {
  ...
  const _BuyDialog({required this.weapon});
      ...
      children: [
        ...
        Container(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            _qtyBtn(icon: Icons.add, active: true, onTap: () => setState(() => _quantity++)),
          ]),
        ),
        const SizedBox(height: 16),
        // total
  ...
  Widget _cancelBtn(Color subText, {VoidCallback? onTap}) {
    return OutlinedButton(
      ...
      child: const Text('Batal',
          style: TextStyle(
              fontFamily: 'Alexandria', fontWeight: FontWeight.w600)),
    );
  }
